...
//		printf("size=%d\n", len);
		...
		sendto(*(int *)sock->data, msg, len, 0, (struct sockaddr *)&name4, sizeof(name4));
		return(0);
	}
#ifdef CONF_USEIPV6
...
/*		printf("send ipv6 to %s.%d.\n", ipv6tostr(targ->targ.ipv6.addr), ntohs(targ->targ.ipv6.port));
		...
		sendto(*(int *)sock->data, msg, len, 0, (struct sockaddr *)&name6, sizeof(name6));
		return(0);
	}
#endif
...
VADDR *udpsock_getaddr(VFILE *sock)
{
	...
	if(UDP_GetSocketAddr(*(int *)sock->data, tmp))
	{
		gcfree(tmp);
		tmp=NULL;
	}
