#define BN_TCP_USE_NODELAY
// #define BN_TCP_USEBUFFER
// #define CONF_USEIPV6
...
//	fhnd=*(int *)sock->buffer;
	...
	if(sz==-1)
	{
		...
		{
		case WSAEWOULDBLOCK:
//			gc_printf("TCP: Read: Would Block\n");
			sz=0;
			break;
		case WSAECONNRESET:
...
//	if(i==SOCKET_ERROR)gc_printf("TCP: write error %d on %d\n",
//		WSAGetLastError(), *(int *)sock->buffer);
	...
	if(i==-1)
	{
		...
		{
		case WSAEWOULDBLOCK:
			gc_printf("TCP: Write: Would Block\n");
			i=0;
			break;
		case WSAECONNRESET:
...
#ifdef BN_TCP_USE_NODELAY
	t=1;
	i=setsockopt(sock, IPPROTO_TCP, TCP_NODELAY,
		(const char *)(&t), sizeof(int));
	if(i==SOCKET_ERROR)gc_printf("TCP: NODELAY: %d on %d\n",
