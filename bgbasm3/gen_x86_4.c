...
// #include <basm/bgbasm_opcat.h>
#include "../include/basm/bgbasm_opcat.h"


char *in_txt="asm/basm_insns.txt";
char *out_c="asm/basm_insns.c";
char *out_h="../include/basm/bgbasm_x86ops.h";
...
int get_opcat(char **str)
{
	...
	if(!strncmp(s, "armsh", 5))	{ i=OPCAT_ARMSH; s+=5; }
...
int main(int argc, char *argv[])
{
	...
	in_txt="asm/basm_insns.txt";
	out_c="asm/basm_insns.c";
	out_h="../include/basm/bgbasm_x86ops.h";
	machl="";
	machh="";
	...
	for(i=1; i<argc; i++)
	{
		if(argv[i][0]=='-')
		{
			if(argv[i][1]=='M')
			{
				...
				sprintf(buf, "asm/basm_insns_%s.txt", s0);
				in_txt=strdup(buf);

				sprintf(buf, "asm/basm_insns_%s.c", s0);
				out_c=strdup(buf);

				sprintf(buf, "../include/basm/bgbasm_%sops.h", s0);
				out_h=strdup(buf);
...
//			opcat[i*16+7]=0;
		...
		if(a[2] && (a[2][0]!='-'))
		{
			...
			while(*s)
			{
				...
				if(!strncmp(s, "even", 4))
					{ j|=OPFL_EVEN; s+=4; continue; }
				if(!strncmp(s, "odd", 3))
					{ j|=OPFL_ODD; s+=3; continue; }
...
//		j=(31-strlen(buf))/8;
		j=(63-(strlen(buf)+strlen("#define BASM_OP_")+strlen(machh)))/8;
		fprintf(fd, "#define BASM_OP%s_%s", machh, buf);
	...
	for(i=0; i<nregs; i++)
	{
		...
		j=(63-(strlen(buf)+strlen("#define BASM_REG_")+strlen(machh)))/8;
		fprintf(fd, "#define BASM_REG%s_%s", machh, buf);
		while((j--)>0)fprintf(fd, "\t");
		fprintf(fd, "%d\n", regidx[i]);
	}
