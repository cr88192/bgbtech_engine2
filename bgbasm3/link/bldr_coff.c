...
int BLNK_COFF_ResolveRelocs(BLNK_Object *ctx, BLNK_COFF_Info *inf, int sec,
	int offs, int cnt)
{
	byte *buf, *ip, *dp;
	char *s;
...
//	printf("Obj1 %s\n", (ctx->flags&BLNK_FL_X86_64)?"x64":"x86");

//	printf("BLNK_COFF_ResolveRelocs(%d, %X, %d):\n", sec, offs, cnt);
	...
	for(i=0; i<cnt; i++)
	{
		...
		if(!(ctx->flags&(BLNK_FL_X86_64|BLNK_FL_ARM)))
		{
			if(*s!='_')continue;
			s++;
		}
...
//		j=inf->sec_rva_off[sec]+j+4;
		...
		if(ctx->flags&BLNK_FL_ARM)
		{
			if(l==0x00) { ty=BLNK_JMP_ABS; j+=8; }
			if(l==0x01) { ty=BLNK_JMP_ABS32; j+=4; }
			if(l==0x02) { ty=BLNK_JMP_ABS32NB; j+=4; }
			if(l==0x03) { ty=BLNK_JMP_ARM_NEAR24; j+=4; }
			if(l==0x04) { ty=BLNK_JMP_THUMB_NEAR22; j+=4; }

			if(l==0x0E) { ty=BLNK_JMP_SECTION; j+=4; }
			if(l==0x0F) { ty=BLNK_JMP_SECREL; j+=4; }
		}else if(ctx->flags&BLNK_FL_X86_64)
		{
			if(l==0x00) { ty=BLNK_JMP_ABS; j+=8; }
			if(l==0x01) { ty=BLNK_JMP_ABS64; j+=8; }
			if(l==0x02) { ty=BLNK_JMP_ABS32; j+=4; }
			if(l==0x03) { ty=BLNK_JMP_ABS32NB; j+=4; }
			if(l==0x04) { ty=BLNK_JMP_NEAR32; j+=4; }
				...
				(l==0x09))
			{
				...
				ty=BLNK_JMP_NEAR32; j+=4;
			}

			if(l==0x0A) { ty=BLNK_JMP_SECTION; j+=4; }
			if(l==0x0B) { ty=BLNK_JMP_SECREL; j+=4; }

		}else
		{
			if(l==0x00) { ty=BLNK_JMP_ABS; j+=4; }
			if(l==0x01) { ty=BLNK_JMP_ABS16; j+=2; }
			if(l==0x02) { ty=BLNK_JMP_NEAR16; j+=2; }
			//3/4/5 undefined
			if(l==0x06) { ty=BLNK_JMP_ABS32; j+=4; }
			if(l==0x07) { ty=BLNK_JMP_ABS32NB; j+=4; }
			//8/9 undefined
			if(l==0x0A) { ty=BLNK_JMP_SECTION; j+=4; }
			if(l==0x0B) { ty=BLNK_JMP_SECREL; j+=4; }

			if(l==0x14) { ty=BLNK_JMP_NEAR32; j+=4; }
		}

//		printf("\t%s %08X %d\n", s, j, ty);

		BLNK_EmitGotoPos(ctx, s, ty, j);
	}
...
//	FILE *fd;
		...
		)
	{
		if((buf[0]==0x7F) && (buf[1]=='E') &&
			(buf[2]=='L') && (buf[3]=='F'))
		{
			ctx=BLNK_ELF_LoadObjectBuf(name, buf, sz);
			return(ctx);
		}
	...
	inf=(BLNK_COFF_Info *)malloc(sizeof(BLNK_COFF_Info));
	memset(inf, 0, sizeof(BLNK_COFF_Info));

//	inf->name=blnk_strdup(name);
...
//	printf("Load Sections\n");
	...
	ctx=BLNK_NewContext();
	if(name)ctx->name=blnk_strdup(name);

	if((buf[0]==0x64) && (buf[1]==0x86))
		ctx->flags|=BLNK_FL_X86_64;

	if((buf[0]==0xC0) && (buf[1]==0x01))
		ctx->flags|=BLNK_FL_ARM;
	if((buf[0]==0xC2) && (buf[1]==0x01))
		ctx->flags|=BLNK_FL_ARM|BLNK_FL_THUMB;
...
//		(ctx->flags&BLNK_FL_X86_64)?"x64":"x86");
	...
	for(i=0; i<inf->nsecs; i++)
	{
		BLNK_COFF_ResolveRelocs(ctx, inf, i,
			inf->sec_rlc_off[i],
			inf->sec_rlc_num[i]);
	}
...
//	FILE *fd;
	...
	if(!fd)
	{
		printf("BLNK_COFF_LoadObject: fail open\n");
		return(NULL);
	}
	...
	ctx=BLNK_COFF_LoadObjectBuf(name, buf, i);
	if(!ctx)return(NULL);
	BLNK_LinkModule(ctx);
...
//	printf("BLNK_COFF_LoadLibrary '%s'\n", name);
	...
	{
		printf("BLNK_COFF_LoadLibrary: bad archive magic\n");
		return(-1);
	}
	...
	while(cs<ce)
	{
		...
		ctx=BLNK_COFF_LoadObjectBuf(nbuf, cs+60, i);
		if(ctx) BLNK_LinkModuleQueue(ctx);
...
//	FILE *fd;
	...
	if(!buf)
	{
//		printf("BLNK_COFF_LoadLibrary: fail open '%s'\n", name);
		return(-1);
	}

//	printf("BLNK_COFF_LoadLibrary: Open '%s'\n", name);
	i=BLNK_COFF_LoadLibraryBuf(name, buf, sz);
	return(i);
}
...
int BLNK_COFF_LoadLibrary(char *name)
{
	...
	if(!fd)
	{
		printf("BLNK_COFF_LoadLibrary: fail open\n");
		return(-1);
	}
...
int blnk_coff_idxstr(char *strs, char *str)
{
	...
	strcpy(s, str);
	s+=strlen(s)+1;
	blnk_coff_set32((byte *)strs, s-strs);
	return(i);
}
...
int blnk_coff_idxstr2(char *strs, char *str, int fl)
{
	...
	if(fl&1)
	{
		i=blnk_coff_idxstr(strs, str);
		return(i);
	}

	sprintf(buf, "_%s", str);
	i=blnk_coff_idxstr(strs, buf);
	return(i);
}
...
{
	char tb[256], tb1[256];
	struct blnk_coff_hdr_s *hdr;
	struct blnk_coff_sec_s *sec_text;
	struct blnk_coff_sec_s *sec_data;
	struct blnk_coff_sym_s *syms;
	struct blnk_coff_reloc_s *rlc_text;
	struct blnk_coff_reloc_s *rlc_data;
...
//	dsz=ctx->dp-ctx->data;
	...
	hdr=(struct blnk_coff_hdr_s *)buf;

	if(ctx->flags&BLNK_FL_X86_64)
	{
		blnk_coff_set16(hdr->mach, 0x8664);
		x64=1;
	}else
	{
		blnk_coff_set16(hdr->mach, 0x014C);
		x64=0;
	}

	blnk_coff_set16(hdr->num_sec, 2);
	blnk_coff_set16(hdr->flags, 0x0104);
	ct+=sizeof(struct blnk_coff_hdr_s);

	sec_text=(struct blnk_coff_sec_s *)ct;
	ct+=sizeof(struct blnk_coff_sec_s);

	sec_data=(struct blnk_coff_sec_s *)ct;
	ct+=sizeof(struct blnk_coff_sec_s);

	strcpy((char *)sec_text->name, ".text");
	strcpy((char *)sec_data->name, ".data");
	blnk_coff_set32(sec_text->flags, 0x60000020);
	blnk_coff_set32(sec_data->flags, 0xC0000040);
	...
	blnk_coff_set32(sec_text->raw_offs, toffs);
	blnk_coff_set32(sec_text->raw_size, tsz);

	blnk_coff_set32(sec_data->raw_offs, doffs);
	blnk_coff_set32(sec_data->raw_size, dsz);
	...
	syms=(struct blnk_coff_sym_s *)ct;
	blnk_coff_set32(hdr->sym_pos, i);
	//dunno size/number yet


	strs=(char *)malloc(1<<16);
	memset(strs, 0, 1<<16);
	blnk_coff_set32((byte *)strs, 5);

	for(i=0; i<ctx->n_labels; i++)
	{
		j=blnk_coff_idxstr2(strs, ctx->label_name[i], x64);
		blnk_coff_set32(syms[i].name+4, j);

		j=(ctx->label_pos[i]>>24);
		k=ctx->label_pos[i]&0xFFFFFF;
		blnk_coff_set16(syms[i].sec, j);
		blnk_coff_set32(syms[i].value, k);

		if(j==1)blnk_coff_set16(syms[i].type, 0x20);
		syms[i].cls=2;
		syms[i].aux=0;
	}
	...
	{
		j=blnk_coff_idxstr2(strs, ctx->goto_name[i], x64);
		for(k=0; k<nsym; k++)
		{
			l=blnk_coff_get32(syms[k].name+4);
			if(j==l)break;
		}
		if(k<nsym)continue;

		k=nsym++;
		blnk_coff_set32(syms[k].name+4, j);
		syms[k].cls=2;
		syms[k].aux=0;
	}

	blnk_coff_set32(hdr->sym_num, nsym);
	ct+=nsym*sizeof(struct blnk_coff_sym_s);

	i=blnk_coff_get32((byte *)strs)+1;
	memcpy(ct, strs, i);
	free(strs);
	strs=(char *)ct;
	ct+=i;
	...
	i=ct-buf; i=(i+15)&(~15); ct=buf+i;
	rlc_text=(struct blnk_coff_reloc_s *)ct;

	blnk_coff_set32(sec_text->rlc_offs, i);
	...
	for(i=0; i<ctx->n_gotos; i++)
	{
		...
		if((ctx->goto_type[i]&15)==BLNK_JMP_ABS16)j-=2;
		if((ctx->goto_type[i]&15)==BLNK_JMP_ABS32)j-=4;
		if((ctx->goto_type[i]&15)==BLNK_JMP_ABS64)j-=8;

		if((ctx->goto_type[i]&15)==BLNK_JMP_NEAR16)j-=2;
		if((ctx->goto_type[i]&15)==BLNK_JMP_NEAR32)j-=4;

//		blnk_coff_set32(rlc_text[ntrlc].addr, j-4);
		blnk_coff_set32(rlc_text[ntrlc].addr, j);
		...
		blnk_coff_set32(rlc_text[ntrlc].sym, k);

		if(x64&1)
		{
			j=0x0002;
			if((ctx->goto_type[i]&15)==BLNK_JMP_ABS64)j=0x0001;
			if((ctx->goto_type[i]&15)==BLNK_JMP_ABS32)j=0x0002;
			if((ctx->goto_type[i]&15)==BLNK_JMP_NEAR32)j=0x0004;
		}else
		{
			j=0x0006;
			if((ctx->goto_type[i]&15)==BLNK_JMP_ABS32)j=0x0006;
			if((ctx->goto_type[i]&15)==BLNK_JMP_NEAR32)j=0x0014;
		}
		blnk_coff_set16(rlc_text[ntrlc].type, j);

		ntrlc++;
	}

	blnk_coff_set16(sec_text->rlc_num, ntrlc);
	ct+=ntrlc*sizeof(struct blnk_coff_reloc_s);


	i=ct-buf; i=(i+15)&(~15); ct=buf+i;
	rlc_data=(struct blnk_coff_reloc_s *)ct;

	blnk_coff_set32(sec_data->rlc_offs, i);
	...
	for(i=0; i<ctx->n_gotos; i++)
	{
		...
		j=ctx->goto_pos[i]&0xFFFFFF;
		blnk_coff_set32(rlc_data[ndrlc].addr, j-4);
		...
		blnk_coff_set32(rlc_data[ndrlc].sym, k);

		j=0x0006;
		if((ctx->goto_type[i]&15)==BLNK_JMP_ABS32)j=0x0006;
		if((ctx->goto_type[i]&15)==BLNK_JMP_NEAR32)j=0x0014;
		blnk_coff_set16(rlc_data[ndrlc].type, j);

		ndrlc++;
	}

	blnk_coff_set16(sec_data->rlc_num, ndrlc);
	ct+=ndrlc*sizeof(struct blnk_coff_reloc_s);
...
int BLNK_COFF_StoreObject(char *name, BLNK_Object *ctx)
{
	...
	sz=BLNK_COFF_StoreObjectBuf(ctx, buf, 1<<24);
	if(sz<0)
	{
		free(buf);
		return(sz);
