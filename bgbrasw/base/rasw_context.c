...
// byte *bgbrasw_heap;
// byte *bgbrasw_heaprov;

s64 bgbrasw_heaprem=1LL<<30;

BGBRASW_API void *bgbrasw_malloc(size_t sz)
{
	byte *ptr, *ptr1;

	if(bgbrasw_heaprem<0)
		__debugbreak();
	bgbrasw_heaprem-=sz;
...
BGBRASW_API void BGBRASW_ClearQueryValue(
	BGBRASW_Context *ctx, int id)
{
	if((id<=0) || (id>=(1<<22)))
		return;
	if(!ctx->query_buf[id>>12])
		ctx->query_buf[id>>12]=bgbrasw_malloc(4096*sizeof(u32));
	ctx->query_buf[id>>12][id&4095]=0;
}

BGBRASW_API int BGBRASW_GetQueryValue(
	BGBRASW_Context *ctx, int id)
{
	if((id<=0) || (id>=(1<<22)))
		return(0);
	if(!ctx->query_buf[id>>12])
		return(0);
	return(ctx->query_buf[id>>12][id&4095]);
}
