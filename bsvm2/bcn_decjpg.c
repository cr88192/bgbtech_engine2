...
byte *huff_css;	//current pos in bitstream (input)
...
//	if(ctx->huff_isend)return(0x00);
	...
	if(i==0xFF)
	{
		if((ctx->huff_cs-ctx->huff_css)<0x500)
...
#if 0
		if((ctx->huff_pos>=16) || (ctx->huff_pos<0))
		{
			if((ctx->huff_cs-ctx->huff_css)<0x500)
				printf("Pos2B %d @%06X\n",
...
int PDJPG_FlattenTGA(byte *obuf, byte *rgba, int xs, int ys)
{
	byte *head;
	int i, j, k, n;
...
int PDJPG_StoreTGA(char *name, byte *rgba, int xs, int ys)
{
	FILE *fd;
	byte *tbuf;
	int sz;
	
	tbuf=malloc(xs*ys*4+64);
...
int main()
{
	...
	ibuf=readfile("NanoITX_4_1.jpg", &sz);

	ctx=PDJPG_AllocContext();
	
	t0=clock();
	PDJPG_DecodeBasic(ctx, ibuf, sz, &xs, &ys);
	t1=clock();
	t2=t1-t0;
	printf("Dec JPG %d\n", t2);
	...
	PDJPG_StoreTGA("fibtst_c0.tga", obuf, xs, ys);
}
