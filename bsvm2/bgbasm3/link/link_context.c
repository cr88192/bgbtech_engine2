...
BLNK_Object *BLNK_NewContext()
{
	...
	tmp=(BLNK_Object *)malloc(sizeof(BLNK_Object));
	memset(tmp, 0, sizeof(BLNK_Object));
...
//		ctx->llbl_pos=(int *)realloc(ctx->llbl_pos, j*sizeof(int));
	...
	ctx->label_name[i]=blnk_strdup(name);
	ctx->label_pos[i]=pos;
}
...
//	printf("Emit Goto %s\n", name);
	...
	ctx->goto_name[i]=name?blnk_strdup(name):NULL;
	ctx->goto_pos[i]=pos;
	ctx->goto_type[i]=ty;
}

#if 0
// void BLNK_EmitLabel(BLNK_Object *ctx, char *name)
{
	int i;
	...
	BLNK_EmitLabelPos(ctx, name, i);
}

// void BLNK_EmitGoto(BLNK_Object *ctx, char *name, int ty)
{
	int i;
	...
	BLNK_EmitGotoPos(ctx, name, ty, i);
}
#endif

#if 0
// void BLNK_EmitConst(BLNK_Object *ctx, char *name, long long val)
{
	int i, j;
	...
	ctx->const_name[i]=blnk_strdup(name);
