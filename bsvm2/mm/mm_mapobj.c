/*
 * Stuff for Map Objects.
 *
 * These objects exist as an associative key/value mapping.
 * This mapping is done with a B-Tree like structure.
 */
...
//	hi=(hi>>12)&4095;
	...
	finf=bgbdt_mapobj_finf[c>>8]+(c&255);
	finf->name=bgbdt_mm_strdup(name);
	finf->sig=bgbdt_mm_strdup(sig);
	finf->flsig=bgbdt_mm_strdup(flsig);
	finf->fidx=c;
...
BS2VM_API int BGBDT_MapObj_BindObjvSlotValueName(
...
BS2VM_API int BGBDT_MapObj_QueryObjSlotList(
