...
//			i*sizeof(void *));
		rgn->ri_ptr=bgbdt_mm_malloc(i*sizeof(void *));
		rgn->ri_next=bgbdt_mm_malloc(i*sizeof(int));
		rgn->ri_nptr=1;
		rgn->ri_mptr=i;
	}
	...
	else
	{
		if((rgn->ri_nptr+1)>=rgn->ri_mptr)
		{
			...
			rgn->ri_ptr=bgbdt_mm_realloc(rgn->ri_ptr, i*sizeof(void *));
			rgn->ri_next=bgbdt_mm_realloc(rgn->ri_next, i*sizeof(int));
			rgn->ri_mptr=i;
		}
