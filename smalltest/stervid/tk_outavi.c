...
// volatile FRGL_AVICtx *pdgl_avi_cap;
...
void FRGL_PNG_WriteInt32LE(VFILE *fd, u32 val)
{
	vfputc((val    )&255, fd);
	vfputc((val>> 8)&255, fd);
	vfputc((val>>16)&255, fd);
	vfputc((val>>24)&255, fd);
}

void FRGL_PNG_WriteFourcc(VFILE *fd, u32 val)
{
	vfputc((val    )&255, fd);
	vfputc((val>> 8)&255, fd);
	vfputc((val>>16)&255, fd);
...
void FRGL_AVI_UpdateSize(VFILE *fd, int o)
{
	int i;

	i=vftell(fd);
	vfseek(fd, o, 0);
	FRGL_PNG_WriteInt32LE(fd, i-o-4);
	vfseek(fd, i, 0);
}

FRGL_AVICtx *FRGL_AVI_OpenOutStream2(char *name,
	int w, int h, float fps, int fcc, int auid)
{
	...
	ctx=pdgl_avi_malloc(sizeof(FRGL_AVICtx));
	ctx->fd=fd;
	...
	ctx->avih=pdgl_avi_malloc(sizeof(FRGL_MainAVIHeader));
	ctx->vidh=pdgl_avi_malloc(sizeof(FRGL_AVIStreamHeader));
	ctx->vidf=pdgl_avi_malloc(sizeof(FRGL_BITMAPINFOHEADER));


	ctx->vidh->fccType=RIFF_TAG_vids;

	printf("FRGL_AVI_OpenOutStream: fcc=%08X\n", fcc);
	...
	ctx->vidf->biSize=sizeof(FRGL_BITMAPINFOHEADER);
	ctx->vidf->biWidth=w;
	ctx->vidf->biHeight=h;
	ctx->vidf->biPlanes=1;
	...
	if(auid)
	{
		ctx->audh=pdgl_avi_malloc(sizeof(FRGL_AVIStreamHeader));
		ctx->audf=pdgl_avi_malloc(sizeof(FRGL_WAVEFORMATEX));

		ctx->audh->fccType=FCC_auds;
		ctx->audh->fccHandler=1;
		ctx->audh->dwFlags=0;

		switch(auid&FRGL_AVI_AUID_MASK_RATE)
		{
		case FRGL_AVI_AUID_8K:
			ctx->audh->dwRate=8000; break;
		case FRGL_AVI_AUID_11K:
			ctx->audh->dwRate=11025; break;
		case FRGL_AVI_AUID_16K:
			ctx->audh->dwRate=16000; break;
		case FRGL_AVI_AUID_22K:
			ctx->audh->dwRate=22050; break;
		case FRGL_AVI_AUID_32K:
			ctx->audh->dwRate=32000; break;
		case FRGL_AVI_AUID_44K:
...
//		ctx->audf->nChannels=1;
		ctx->audf->nChannels=(auid&FRGL_AVI_AUID_STEREO)?2:1;
...
//		ctx->audf->wBitsPerSample=8;
	...
	FRGL_PNG_WriteFourcc(ctx->fd, RIFF_TAG_RIFF);
	ctx->osz_avi=vftell(ctx->fd);
	FRGL_PNG_WriteInt32LE(ctx->fd, 0);
	FRGL_PNG_WriteFourcc(ctx->fd, RIFF_TAG_AVI);

	FRGL_PNG_WriteFourcc(ctx->fd, RIFF_TAG_LIST);
	ctx->osz_hdrl=vftell(ctx->fd);
	FRGL_PNG_WriteInt32LE(ctx->fd, 0);
	FRGL_PNG_WriteFourcc(ctx->fd, RIFF_TAG_hdrl);

	i=sizeof(FRGL_MainAVIHeader);
	FRGL_PNG_WriteFourcc(ctx->fd, RIFF_TAG_avih);
	FRGL_PNG_WriteInt32LE(ctx->fd, i);
	...
	FRGL_PNG_WriteInt32LE(ctx->fd, 0);
	FRGL_PNG_WriteFourcc(ctx->fd, RIFF_TAG_strl);

	i=sizeof(FRGL_AVIStreamHeader);
	FRGL_PNG_WriteFourcc(ctx->fd, RIFF_TAG_strh);
	FRGL_PNG_WriteInt32LE(ctx->fd, i);
	ctx->offs_vidh=vftell(ctx->fd);
	vfwrite(ctx->vidh, 1, (i&1)?(i+1):i, ctx->fd);

#if 1
	i=sizeof(FRGL_BITMAPINFOHEADER);
	FRGL_PNG_WriteFourcc(ctx->fd, RIFF_TAG_strf);
	FRGL_PNG_WriteInt32LE(ctx->fd, i);
	...
	{
		i=ctx->sz_vidStrd;
		FRGL_PNG_WriteFourcc(ctx->fd, RIFF_TAG_strd);
		FRGL_PNG_WriteInt32LE(ctx->fd, i);
	...
	FRGL_AVI_UpdateSize(ctx->fd, ctx->osz_strl);
	...
	FRGL_AVI_UpdateSize(ctx->fd, ctx->osz_hdrl);

	FRGL_PNG_WriteFourcc(ctx->fd, RIFF_TAG_LIST);
	ctx->osz_movi=vftell(ctx->fd);
	FRGL_PNG_WriteInt32LE(ctx->fd, 0);
	FRGL_PNG_WriteFourcc(ctx->fd, RIFF_TAG_movi);

	return(ctx);
}

FRGL_AVICtx *FRGL_AVI_OpenOutStream(char *name,
	int w, int h, float fps, int fcc)
{
	return(FRGL_AVI_OpenOutStream2(
		name, w, h, fps, fcc, FRGL_AVI_AUID_22K8BM));
}

void FRGL_AVI_CloseOutStream(FRGL_AVICtx *ctx)
{
	int i;

//	FRGL_JPG_FlushEncodeFast(ctx->vidCtx);

	FRGL_AVI_UpdateSize(ctx->fd, ctx->osz_movi);

#if 1
	FRGL_PNG_WriteFourcc(ctx->fd, RIFF_TAG_idx1);
	FRGL_PNG_WriteInt32LE(ctx->fd, ctx->n_index*4*4);
	for(i=0; i<ctx->n_index; i++)
	{
		FRGL_PNG_WriteFourcc(ctx->fd, ctx->index[i*4+0]);
		FRGL_PNG_WriteInt32LE(ctx->fd, ctx->index[i*4+1]);
		FRGL_PNG_WriteInt32LE(ctx->fd, ctx->index[i*4+2]);
		FRGL_PNG_WriteInt32LE(ctx->fd, ctx->index[i*4+3]);
	}
#endif


	FRGL_AVI_UpdateSize(ctx->fd, ctx->osz_avi);

	if(ctx->audh)
	{
		vfseek(ctx->fd, ctx->offs_audh, 0);
		vfwrite(ctx->audh, 1, sizeof(FRGL_AVIStreamHeader), ctx->fd);

		vfseek(ctx->fd, ctx->offs_audf, 0);
		vfwrite(ctx->audf, 1, sizeof(FRGL_WAVEFORMATEX), ctx->fd);
	}

	vfseek(ctx->fd, ctx->offs_vidh, 0);
	vfwrite(ctx->vidh, 1, sizeof(FRGL_AVIStreamHeader), ctx->fd);

	vfseek(ctx->fd, ctx->offs_vidf, 0);
	vfwrite(ctx->vidf, 1, sizeof(FRGL_BITMAPINFOHEADER), ctx->fd);

	vfseek(ctx->fd, ctx->offs_avih, 0);
	vfwrite(ctx->avih, 1, sizeof(FRGL_MainAVIHeader), ctx->fd);
...
int FRGL_AVI_AddIndex(FRGL_AVICtx *ctx, u32 tag, u32 len, u32 fl)
{
	int i;
...
void FRGL_AVI_EmitCodedFrame(FRGL_AVICtx *ctx, byte *buf,
	int sz, int qf)
{
	int fl;

	fl=0;
	if(!(qf&FRGL_QFL_PFRAME))
		{ fl|=16; }

	FRGL_AVI_AddIndex(ctx, RIFF_TAG_00dc, sz, fl);

	FRGL_PNG_WriteFourcc(ctx->fd, RIFF_TAG_00dc);
	FRGL_PNG_WriteInt32LE(ctx->fd, sz);
...
void FRGL_AVI_EmitAudioFrame(FRGL_AVICtx *ctx)
{
	...
	if(ctx->audh)
	{
		...
		if(ctx->audf->wFormatTag==FRGL_WAVE_FORMAT_IMAADPCM)
		{
//#if !defined(FRGL_DRV) && !defined(FRGL_NODY)
#if 0
			...
			if(!ctx->audSampleBuf)
			{
				FRGL_AVI_AddIndex(ctx, RIFF_TAG_01wb, sz, 16);
				FRGL_PNG_WriteFourcc(ctx->fd, RIFF_TAG_01wb);
				FRGL_PNG_WriteInt32LE(ctx->fd, sz);
		...
		else if(ctx->audf->wFormatTag==FRGL_WAVE_FORMAT_PCM)
		{
			if(!ctx->audSampleBuf)
			{
				FRGL_AVI_AddIndex(ctx, RIFF_TAG_01wb, sz, 16);
...
void FRGL_AVI_EncodeFrame2(FRGL_AVICtx *ctx,
	byte *ibuf, int w, int h, int qf, int clrs)
{
	...
	{
		if(!(qf&(FRGL_QFL_IFRAME|FRGL_QFL_PFRAME)))
		{
//			if(!(ctx->frnum&15))
			if(!(ctx->frnum&63))
				{ qf|=FRGL_QFL_IFRAME; }
		...
		sz=FRGL_Codec_CompressFrame(ctx->vid_codec_ctx,
			ibuf, tbuf, 1<<22, qf, 0, &fl);
		qf&=(~(FRGL_QFL_IFRAME|FRGL_QFL_PFRAME));
		qf|=fl&(FRGL_QFL_IFRAME|FRGL_QFL_PFRAME);
	}

	if(sz==-999)
	{
		ctx->vid_codec_ctx=FRGL_Codec_BeginCompress(
	...
	if(qf&FRGL_QFL_BATCHENCODE)
	{
		FRGL_AVI_EmitCodedFrame(ctx, tbuf, sz, qf);
		FRGL_AVI_EmitAudioFrame(ctx);
		ctx->accDt2=0;
	}else
	{
...
void FRGL_AVI_EncodeFrame(FRGL_AVICtx *ctx,
	byte *ibuf, int w, int h)
{
	FRGL_AVI_EncodeFrame2(ctx, ibuf, w, h, 90, 0);
}

int FRGL_AVI_WriteContextStereoSamples(
	FRGL_AVICtx *ctx, short *buf, int cnt)
{
	int i, j, k;
...
int FRGL_AVI_WriteContextMonoSamples(
	FRGL_AVICtx *ctx, short *buf, int cnt)
{
	int i, j, k;
...
#if 0
int FRGL_AVI_GetEncodeTime()
	{ return(pdgl_avi_enctime); }

int FRGL_AVI_WriteStereoSamples(short *buf, int cnt)
{
	int i, j, k;
...
int FRGL_Tex_SplinePolateRGBA8(byte *src, int w, int h,
...
int FRGL_Tex_ResampleSpline(byte *src, int iw, int ih, byte *dst, int ow, int oh)
{
	float tv[4];
	float xs, ys, xc, yc;
	float ix, iy, fx, fy;
	...
	for(i=0; i<oh; i++)
		for(j=0; j<ow; j++)
	{
		FRGL_Tex_SplinePolateRGBA8(src, iw, ih, j*xs, i*ys, tv);
